...
def purchase_new_pet():
    ...
    if response.status_code == 200:
        data = response.json()
        print(f'Pet {PET_ID} purchased:', data)
...
def delete_order():
    ...
    ok_json = {"code": 200, "type": "unknown", "message": "10"}
    response = s.delete(urljoin(URL, str(ORDER_ID)))
    assert response.status_code == 200
    assert response.json() == ok_json
    if response.status_code == 200:
        data = response.json()
        print(f'Order {ORDER_ID} deleted:', data)
        print("Status code is ", response.status_code)
    else:
        print("Error retrieving data:", response.text)
...
def delete_non_existent_order():
    ...
    ok_json = {"code": 404, "type": "unknown", "message": "Order Not Found"}
    response = s.delete(urljoin(URL, str(ORDER_ID)))
    assert response.status_code == 404
    assert response.json() == ok_json
    if response.status_code == 404:
        data = response.json()
        print(f'Order {ORDER_ID} not found:', data)
        print("Status code is ", response.status_code)
    else:
        print("Error retrieving data:", response.text)
...
def find_non_existent_order():
    ...
    ok_json = {"code": 1, "type": "error", "message": "Order not found"}
    response = s.get(urljoin(URL, str(ORDER_ID)))
    assert response.status_code == 404
    assert response.json() == ok_json
    if response.status_code == 404:
